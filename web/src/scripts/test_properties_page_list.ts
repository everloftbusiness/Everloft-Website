...
async function main() {
  console.log("Simulating listPublicActiveProperties(100) for /properties page...");
  ...
  (properties || []).forEach((p, idx) => {
    ...
    if (!coverUrl) {
      console.log(`❌ [${idx + 1}] "${p.name}" (Slug: ${p.slug}) ➔ COVER IS NULL (SHOWS PHOTO COMING SOON)`);
    } else {
      console.log(`✅ [${idx + 1}] "${p.name}" (Slug: ${p.slug}) ➔ Cover: ${coverUrl.slice(0, 70)}...`);
    }
  });
}
