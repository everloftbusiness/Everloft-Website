...
async function main() {
  console.log("Syncing all property preset rules to match STANDARD_HOUSE_RULES exact strings...");
  ...
  for (const p of props) {
    if (p.name.includes("Electronic City") || p.name.includes("3BHK")) {
      ...
      console.log(`  Synced Standard rules for Property: ${p.name} (${p.id})`);
    }
  }

  console.log("\n🎉 ALL property preset rules mapped to STANDARD_HOUSE_RULES successfully!");
}
